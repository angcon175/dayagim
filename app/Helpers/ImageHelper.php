...
function uploadAdImage(?object $file, string $path, $watermark = false): string
{
    ...
    if ($watermark && setting('watermark_status')) {
        ...
        $updated_img->resize(850, null, function ($constraint) { $constraint->aspectRatio(); });
        $updated_img->save(public_path('/uploads/' . $path . '/') . $fileName);
        return "uploads/$path/" . $fileName;
    } else {

        $fileName = time() . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
        $fileup = public_path('/uploads/' . $path . '/'). $fileName;
        Image::make($file)->resize(850, null, function ($constraint) { $constraint->aspectRatio(); })
        ->save($fileup);
        return "uploads/$path/" . $fileName;
    }
}
