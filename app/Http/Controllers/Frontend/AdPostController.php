...
class AdPostController extends Controller
{
    ...
    public function storePostStep1(Request $request)
    {
        ...
        $validatedData['featured'] = $request->featured;
        ...
        try {
            if (empty(session('ad'))) {
                $ad = new Ad();
                $slug = Str::slug($request->title, '-', 'PR');
                $ad['slug'] = $slug;
                $ad->fill($validatedData);
                $ad->product_status_id=$request->product_status_id;
                $ad->delivery_sell_id=$request->delivery_sell_id;
    ...
    public function storePostStep2(Request $request)
    {
        ...
        $validatedData['show_phone'] = $request->show_phone;
        $validatedData['phone_2'] = $request->phone_2;
    ...
    public function UpdatePostStep1(Request $request, Ad $ad)
    {
        ...
        $ad->update([
            ...
            'delivery_sell_id'=>$request->delivery_sell_id,
        ]);
    ...
    public function updatePostStep3(Request $request, Ad $ad)
    {
        ...
        if ($thumbnail) {
            $url = uploadaDImage($thumbnail, 'addss_image', true);
            $ad->update(['thumbnail' => $url]);
            if ($old_thumb) {
               @unlink($old_thumb);
            }
