<div class="{{ $className }}">
    <div class="cards cards--one {{ $ad->featured ? 'cards--highlight' : '' }}">
        @if($ad->featured)
             <a href="{{ route('frontend.addetails',$ad->slug) }}" class="featured_tag">
               <span>Featured</span>
            </a>
        @endif
        ...
        <div class="cards__info">
            <div class="cards__info-top">
                ...
                @if ($ad->delivery_sell_id == "0")
                <span class="text-primary">{{__('For Delivery')}}</span>
                @else
                <span class="text-primary">{{__('For Sale')}}</span>
                @endif
            ...
            <div class="cards__info-bottom">
                <h6 class="cards__location text--body-4">
                    @if ($ad->region || $ad->country)
                    <span class="icon">
                        <x-svg.location-icon width="20" height="20" stroke="#50AEE7" />
                    </span>
